...
traits<-traits %>% filter(Red_Sea == 1)
...
uniqe_sp_2 <- uniqe_sp %>%
  mutate("Species" = coalesce(Species_updated_name_fishbase,v_names)) %>% 
  mutate("Species" = coalesce(Species_updated_name_fishbase,uniqe_sp))
...
```{r}
uniqe_sp_2$Species[uniqe_sp_2$uniqe_sp=="Zebrasoma veliferum"]<-"Zebrasoma desjardinii"

uniqe_sp_2<-uniqe_sp_2 %>% select(1,5)
